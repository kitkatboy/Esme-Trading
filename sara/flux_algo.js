...
event.on("algo", function(){
while(server.nbr<1) // TODO 39
{
  
server.calcul(server.nbr++); // appel a la fonction qui exctrai les données de l'entreprise i
...
if(data.jour.length>5) // SI ON A UNE SEMAINE DE DONNEES
{
	...
	derniere_valeure_semaine=derniere_valeure_semaine.split(',');
	derniere_valeure_semaine=derniere_valeure_semaine[0]+"."+derniere_valeure_semaine[1]
	var premiere_valeure_semaine=data.jour[data.jour.length-6].heure[0].valeure
	premiere_valeure_semaine=premiere_valeure_semaine.split(',');
	premiere_valeure_semaine=premiere_valeure_semaine[0]+"."+premiere_valeure_semaine[1]
	var coef_dirrecteur_semaine = Number(nbr_valeure_semaine) / (Number(derniere_valeure_semaine) - Number(premiere_valeure_semaine));
	...
	derniere_valeure_jour=derniere_valeure_jour.split(',');
	derniere_valeure_jour=derniere_valeure_jour[0]+"."+derniere_valeure_jour[1]
	var premiere_valeure_jour=data.jour[data.jour.length-1].heure[0].valeure
	premiere_valeure_jour=premiere_valeure_jour.split(',');
	premiere_valeure_jour=premiere_valeure_jour[0]+"."+premiere_valeure_jour[1]
	var coef_dirrecteur_jour = Number(nbr_valeure_jour) / (Number(derniere_valeure_jour) - Number(premiere_valeure_jour));
...
var vecteur_jour={"valeure" :[]};
	...
	{
		vecteur_semaine.valeure[val] = Number(droite_semaine.coef_dirrecteur) * (val) + Number(droite_semaine.valeure_origine);	
	}

	for(tps=0; tps<droite_jour.nbr_valeure_jour; tps++)
	{
		vecteur_jour.valeure[tps]= Number(droite_jour.coef_dirrecteur) * (5*tps) + Number(droite_jour.valeure_origine);
	}
...
var cac_40={"entreprise": []};
var nbr_jours_semaine= data.jour.length-1;
var nbr_valeur_jour= data.jour[data.jour.length-1].heure.length;
...
var tmp=0;
var json={};
cac_40.nom_entreprise=data.nom_entreprise;
//console.log(vecteur_jour);
//console.log(vecteur_semaine);
//console.log(require("util").inspect(vecteur_semaine));
for(i=0; i< nbr_jours_semaine; i++)
	...
	cac_40.entreprise[num_entreprise] = json;
	if(cac_40.entreprise)
	{
		event.emit("enregistrement", cac_40);
	}
...
event.on("enregistrement", function(cac_40){
	fs.writeFile("cac_40", JSON.stringify(cac_40), 'utf8', 'a+', function(err){
		if(err) throw err;
		console.log('we just saved the buffer in a file');
		//event.emit("algo");// DECLANCHEMENT D4UN EVENEMENT POUR PASS2 A L4ENTREPRISE SUIVANTE //TODO
		}); 
